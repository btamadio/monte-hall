...
* [`src/cljs/monte_hall/`](src/cljs/monte_hall/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/monte_hall/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `monte-hall` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/monte_hall/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns monte-hall.example
  (:require [monte-hall.config :as config])
